...
public class DB_Manager : MonoBehaviour
{
    ...
    public void Register()
    {
        if (IsValid())
        {
            string cmd = "INSERT INTO `users` (`id`, `pseudo`, `email`, `password`, `firstname`, `lastname`, `gender`, `phonenumber`, `country`, `address`, `city`, `zipcode`, `birthmonth`, `birthday`, `birthyear`, `profile`, `points`) VALUES(NULL, '" + RPseudo.text + "', '" + REmail.text + "', '" + Md5Sum(RPassword.text) + "', '" + RFirstName.text + "', '" + RLastName.text + "', '" + selectedGender.text + "', '" + RPhoneNumber.text + "', '" + selectedCountry.text + "', '" + RAddress.text + "', '" + RCity.text + "', '" + RZipCode.text + "', '" + selectedBirthMonth.text + "', '" + selectedBirthDay.text + "','" + selectedBirthYear.text + "','" + selectedProfile.text + "','0')";
